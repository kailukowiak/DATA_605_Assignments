...
Our F statsitic was calucalted with one independant variable and 188 degrees of freedom. it has a significance of 
`r fpVal` which is higly significant. This means that the one included variable is significant. (Similarly a t-test could have been performed.)
...
The standard error is: `r standErr[2]` which is the standard deviation of the sampling distrobution. Lower numbers mean more significance (because they directly relate to the t test.). 

I did not include the SE of the intercept because intercepts are harder to interpret. 

The p-value for the model is `r sum1$coefficients[6]` which means that it is higly significant. The same logic holds true in that I won't include the intercept. 
...
`r fpVal2` which is higly significant (baisically zero) which is much better than the original.
...
The standard error is: `r standErr2[2]` which is the standard deviation of the sampling distrobution. Again this is better than the original (however it needs to be interpreted differently because the variables are different.)

I did not include the SE of the intercept because intercepts are harder to interpret. 

The p-value for the model is `r sum2$coefficients[6]` which means that it is higly significant. The same logic holds true in that I won't include the intercept. 
...
This model (with the interaction term) and aditional variable performs better than the original with a higher 
...
This makes little sense because one would not expect a country with the lowest total expendature (around 14) to have around the highest percent of MDs. The model should only be interpreted locally. 
